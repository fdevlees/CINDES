...
class Converter():
    ...
    def read_zmalist( self, zma):
        '''read zmat from list argument as represented by CINDES program'''

        self.zmatrix = []
        self.zmatrix.append( [ zma[0][0], [], self.masses[zma[0][0]] ] )
        name, atom1, distance = zma[1]
        self.zmatrix.append( [ name, [ [int(atom1) - 1, float(distance)], [], [] ], self.masses[name] ] )
        name, atom1, distance, atom2, angle = zma[2]
        self.zmatrix.append( [ name, [[ int(atom1) - 1, float(distance) ], [int(atom2) - 1, m.radians( float(angle) ) ], []], self.masses[name] ] )
        for item in zma[3:]:
            # Get the components of each line, dropping anything extra
