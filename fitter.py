...
class Dataset(): #abstract data class
    ...
    def analyze(self,data):
        ...
        for i in range(ldat):
            datel = data[i]
            dcolumn = [ item[1] for item in datel ]
    ...
    def openfile(self,file='tablebin'):
        ...
        if True:
            while True:
                try:
                    ruwdata.append(pickle.load(file))
                except EOFError:
                    break
...
class Adamantane(Dataset):
    ...
    #initially don't need an __init__ function now
...
def main(args):
    ...
    for i in range(args.times[0]):
        ...
        if args.ridge:
            ...
            clf_Ridge = myrun.linreg(model='Ridge', alpha=args.ridge)
            if args.analyse:
...
@log_io()
def regression(table, indices,identify, **kwargs):
