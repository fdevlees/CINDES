...
class Run(object):
    ...
    def printer(self)
        logging.info(pprint.pformat('starttime' + str(self.starttime)))
        logging.info(pprint.pformat('script' + self.script))
        logging.info(self.directory)
        logging.info(self.node)
        logging.info(pprint.pformat('pid: ' + str(self.pid)))
        logging.info(pprint.pformat('ppid: ' + str(self.ppid)))
        
    def set_table(self):
        if self.param['restart']>0:
    ...
    def set_maximum(self, table):
        if self.param['restart'] >= 3:
            tabledict = dict(table)
            self.maximum = [ self.param['startconf'] , tabledict[self.param['startconf'] ] ]
            logging.info("maximum:"+ str(self.maximum))
        else:
            self.maximum = 0
        return self.maximum

    def standard_procedure(self,array):
        
        # print initial run values
        self.printer()

        self.param, path = setup_filesystem(self.param)

        startconf = get_startconf(self.param,array)
        self.bcok=0
 
        self.run_main(array,startconf)
 
        return

    def run_main(self,array,startconf):

        #RESTART PROPERTIES INPUT: param
        table = self.set_table(self.param)
        # OUTPUT table
        
        #zmatrix reading and splitting
        corematrix0, activematrix0, passivematrix0 = geometry(file,self.param)
        ...
        while True:
            ...
            for l in range(len(sequence)):
                ...
                if not param['nosub']==1:
                    data = submittingprocedure(corematrix0,activematrix0,passivematrix0,
                                               ...
                                               param['path'],
                                               data,
                                               param) # here call submitting procedure
                else:
                ...
                table = loggings(data,table,count,k,l)
            ...
            self.maximum, maxsite,converged = runtest(param, self.maximum, maxsite, count, bcok)
...
def submittingprocedure(core,active,passive,confs,indices,path,data,fileparameters):
    ...
    if fileparameters['program'] in ['ORCA','orca','Orca']:
        ...
        data = orcafunctions.submittingprocedure(core,active,passive,confs,indices,data,fileparameters)
        #---           -------------           ---------------           --------------           ---#
...
def montecarloprocedure(fileparameters, subarray, max, table): #version 4/10/2015
    ...
    cmaximum = zcon.indtocon(max[0]) # maximum is in index format. change to confformat
...
def geometry(file,param):
    ...
    return coremat, activemat, passivemat
...
def runtest(param, maximum, maxsite, count, bcok):
    converged=0
    ...
    if (count > 1 and bcok) or param['restart']>=3: #BCOK is a test of the boundary condition is already fullfilled
        if maximum[1] == maxsite[1]: 
            ...
            if param['montecarlo'] == 0:
               ...
               maxsite = montecarloprocedure(param, array, maximum, table)
...
def loggings(data,table,count,k,l):
    #--- LOGGINGS: CYCLESINFO
...
def generate_procedure(param,array):
    ...
    #get structure
    corematrix0, activematrix0, passivematrix0 = geometry(file,param) 
    get_startconf(param,array)
    return
...
if __name__ == "__main__":
    ...
    if param['procedure'] == 'standard':
        myrun = Run('myrun',param)
        myrun.standard_procedure(array)
